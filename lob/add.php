...
<script>
    ...
    document.addEventListener('keypress', inputting)
    document.removeEventListener('keypress', inputting);
    */
    let send_datas = []
    for(let breakk = true; breakk = true; print('next')){
        var input = prompt()
        if(!input){
            breakk = false
        } else {
            send_datas.push( (async()=>{await ( await fetch(`https://www.googleapis.com/books/v1/volumes?q=isbn:${input}`) ).json().items[0]})() )
        }
    }
    const items = send_datas.map(item => {
